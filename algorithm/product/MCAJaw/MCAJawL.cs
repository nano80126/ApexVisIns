...
{
    public class MCAJawL : MCAJawAlgorithm
    {
        #region 單位換算
#if true
        public override double Cam1Mag { get; set; } = 0.21741;
        public override double Cam2Mag { get; set; } = 0.25433;
        ...
        #region 光源參數
        readonly ushort[] Cam1Light = new ushort[] { 128, 0 };
        readonly ushort[] Cam2Light = new ushort[] { 0, 160 };
        readonly ushort[] Cam3Light = new ushort[] { 320, 320 };
        #endregion
        ...
        {
            { "有料檢知", new Rect(200, 525, 800, 30) },
            { "有料檢知2", new Rect(720, 940, 1000, 30) },
            { "有料檢知3", new Rect(1800, 310, 400, 60) },
            { "粗定位左", new Rect(180, 370, 250, 340) },
            { "粗定位右", new Rect(770, 370, 250, 340) },
            { "治具定位", new Rect(460, 900, 160, 100) },
            { "後開位置", new Rect(620, 1550, 1200, 60) },
            { "側面定位", new Rect(1800, 190, 240, 120) }
        };
        #endregion
            ...
            try
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    MainWindow.Dispatcher.Invoke(() =>
                    {
                        for (int j = 0; j < (i == 0 ? 2 : 3); j++)
                        {
                            ...
                            // 等待 Trigger Ready
                            bool ready = cam1.Camera.WaitForFrameTriggerReady(50, TimeoutHandling.Return);
                            if (!ready)
                            {
                                j--;
                                continue;
                            ...
                            using IGrabResult grabResult = cam1.Camera.StreamGrabber.RetrieveResult(50, TimeoutHandling.Return);
                    ...
                    MainWindow.Dispatcher.Invoke(() =>
                    {
                        for (int j = 0; j < 2; j++)
                        {
                            ...
                            // 等待 Trigger Ready
                            bool ready = cam2.Camera.WaitForFrameTriggerReady(50, TimeoutHandling.Return);
                            if (!ready)
                            {
                                j--;
                                continue;
                            ...
                            if (grabResult?.GrabSucceeded == true)
                            {
                                ...
                                MainWindow.ImageSource2 = mat.ToImageSource();
                                //Cv2.ImShow($"img{DateTime.Now.Millisecond}", mat);
                            }
                            else { j--; }
                        }
                    });
                ...
                MainWindow.Dispatcher.Invoke(() =>
                {
                    for (int j = 0; j < 3; j++)
                    {
                        ...
                        bool ready = cam3.Camera.WaitForFrameTriggerReady(50, TimeoutHandling.Return);
                        ...
                        cam3.Camera.ExecuteSoftwareTrigger();
                        IGrabResult grabResult = cam3.Camera.StreamGrabber.RetrieveResult(50, TimeoutHandling.Return);
        ...
        public override void JawInsSequenceCam1(Mat src, List<JawSpecSetting> specList = null, Dictionary<string, List<double>> results = null)
        {
            ...
            try
            {
                ...
                if (spec?.Enable == true && results != null)
                {
                    GetContourPlanarY2(src, baseL, JawPos.Left, LX, out PlanarR);
                    GetContourPlanarY2(src, baseR, JawPos.Right, RX, out PlanarL);

                    double c_005 = (Math.Abs(PlanarR - PlanarL) * Cam1Unit) + spec.Correction + spec.CorrectionSecret;
                    lock (results)
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                ...
                if (spec?.Enable == true && results != null)
                {
                    ...
                    if (PlanarR == 0)
                    {
                        Cal012DistanceValue(src, baseL, JawPos.Left, LX, (subContourPts[0].Y + subContourPts[1].Y) / 2, out d_012R, spec.Correction + spec.CorrectionSecret);
                    }
                    else
                    {
                        d_012R = (Math.Abs(((subContourPts[0].Y + subContourPts[1].Y) / 2) - PlanarR) * Cam1Unit) + spec.Correction + spec.CorrectionSecret;
                    }
                ...
                else if (results == null)
                {
                    ...
                    if (PlanarR == 0)
                    {
                        Cal012DistanceValue(src, baseL, JawPos.Left, LX, (subContourPts[0].Y + subContourPts[1].Y) / 2, out d_012R);
                    }
                    else
                    {
                ...
                if (spec?.Enable == true && results != null)
                {
                    ...
                    if (PlanarL == 0)
                    {
                        Cal012DistanceValue(src, baseR, JawPos.Right, RX, (subContourPts[2].Y + subContourPts[3].Y) / 2, out d_012L, spec.Correction + spec.CorrectionSecret);
                    }
                    else
                    {
                        d_012L = (Math.Abs(((subContourPts[2].Y + subContourPts[3].Y) / 2) - PlanarL) * Cam1Unit) + spec.Correction + spec.CorrectionSecret;
                    }
                ...
                else if (results == null)
                {
                    ...
                    if (PlanarL == 0)
                    {
                        Cal012DistanceValue(src, baseR, JawPos.Right, RX, (subContourPts[2].Y + subContourPts[3].Y) / 2, out d_012L);
                    }
                    else
                    {
                        d_012L = Math.Abs(((subContourPts[2].Y + subContourPts[3].Y) / 2) - PlanarL) * Cam1Unit;
                    }
                    Debug.WriteLine($"{nameof(d_012L)}: {d_012L:F5}");
                }
                #endregion

                #region 取得影像上方角點 (計算 0435 ~ 0405 用)
                // 取得角點 左 (實際為右)
                ...
                #region 計算 0.013 左溝槽深度 (實際上是右)
                spec = specList?[3];
                if (spec?.Enable == true && results != null)
                {
                    Cal013ThinknessValue(src, baseL, LX, out double LX013, out double d_013R, spec.Correction + spec.CorrectionSecret);
                    lock (results)
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                ...
                {
                    Cal013ThinknessValue(src, baseL, LX, out double LX013, out double d_013R);
                    Debug.WriteLine($"溝深 {nameof(d_013R)}: {d_013R:F5}, {nameof(LX013)}: {LX013}");
                }
                #endregion

                #region 計算 0.013 右溝槽深度 (實際上是左)
                spec = specList?[4];
                if (spec?.Enable == true && results != null)
                {
                    Cal013ThinknessValue(src, baseR, RX, out double RX013, out double d_013L, spec.Correction + spec.CorrectionSecret);
                    lock (results)
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                ...
                {
                    Cal013ThinknessValue(src, baseR, RX, out double RX013, out double d_013L);
                    Debug.WriteLine($"溝深 {nameof(d_013L)}: {d_013L:F5}, {nameof(RX013)}: {RX013}");
                }
                #endregion
        ...
        public override void JawInsSequenceCam2(Mat src, List<JawSpecSetting> specList = null, Dictionary<string, List<double>> results = null)
        {
            ...
            try
            {
                ...
                {
                    Cal174DistanceValue2(src, JigPosY, RX, JawPos.Right, out d_174R, spec.Correction + spec.CorrectionSecret);
                    lock (results)
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                        results[spec.Key].Add(d_174R);
                    }
                }
                else if (results == null)
                {
                    Cal174DistanceValue2(src, JigPosY, RX, JawPos.Right, out d_174R);
                    Debug.WriteLine($"{nameof(d_174R)}: {d_174R:F5}");
                }
                #endregion
                ...
                if ((spec?.Enable == true || specList?[2].Enable == true) && results != null)
                {
                    Cal174DistanceValue2(src, JigPosY, LX, JawPos.Left, out d_174L, spec.Correction + spec.CorrectionSecret);
                    lock (results)
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                        results[spec.Key].Add(d_174L);
                    }
                }
                else if (results == null)
                {
                    Cal174DistanceValue2(src, JigPosY, LX, JawPos.Left, out d_174L);
                    Debug.WriteLine($"{nameof(d_174L)}: {d_174L:F5}");
                }
                #endregion
                ...
                if (spec?.Enable == true && results != null)
                {
                    ...
                    {
                        if (!results.ContainsKey(spec.Key)) { results[spec.Key] = new List<double>(); }
                        results[spec.Key].Add(d_174R + d_174L);
                    }
                }
                #endregion
        ...
        {
            // 計算 roi
            Rect leftRoi = new(leftPt.X - 50, leftPt.Y - 100, 30, 40);
            Rect rightRoi = new(rightPt.X + 20, rightPt.Y - 100, 30, 40);
            ...
            Debug.WriteLine($"前開:");
            Debug.WriteLine($"{Math.Abs(leftX - rightX)} px, Distance: {distance}");
        ...
        public void GetContourCornerPoint(Mat src, Point basePt, double baseX, JawPos roiPos, out Point p1, out Point p2)
        {
            ...
            {
                case JawPos.Left:
                    roi = new Rect((int)baseX + 2, basePt.Y - 45, 33, 60);
                    break;
                case JawPos.Right:
                    roi = new Rect((int)baseX - 35, basePt.Y - 45, 33, 60);
                    break;
                default:
                    roi = new Rect();
                    break;
        ...
        /// <param name="roiPos">Jaw 左、又</param>
        ...
        public void GetContourPlanarY(Mat src, Point basePoint, JawPos roiPos, double X, out double PosY)
        {
            ...
            Methods.GetRoiCanny(src, roi, 75, 140, out Mat canny);
            ...
            // 計算最長線段
            double max = lineH.Max(line => line.Length());
            // 過濾極端值
            lineH = lineH.Where(line => line.Length() == max || Math.Abs(((line.P1.Y + line.P2.Y) / 2) - botY) < 1.5).ToArray();
            // 計算總長
        ...
        /// <param name="src">來源影像</param>
        /// <param name="basePoint">基準點</param>
        /// <param name="roiPos">Jaw 左右</param>
        /// <param name="X">ROI X (從前開取得)</param>
        /// <param name="PosY">平面 Y 座標</param>
        public unsafe void GetContourPlanarY2(Mat src, Point basePoint, JawPos roiPos, double X, out double PosY)
        {
            // 計算 ROI
            Rect roi = new Rect();
            //
            ...
            double[] grayArr;   // dp 紀錄灰階值
            double grayAbs;     // 紀錄斜率最大時的灰階
            int tempY = 0;      // 斜率最大的 Y 座標 
            // ? : 切換往左或往右掃描
            for (int i = iStart; roiPos == JawPos.Left ? i < roiMat.Width : i >= 0; i += iStep)
            {
                ...
                for (int j = 0; j < roiMat.Height; j++)
                {
                    ...
                    if (grayArr[j] > grayArr[k] && Math.Abs(grayArr[j] - grayArr[k]) > grayAbs)
                    {
                        tempY = j;
                        grayAbs = Math.Abs(grayArr[j] - grayArr[k]);
                    }
        ...
        public void GetCornerPoint(Mat src, Point basePt, double baseX, JawPos roiPos, out Point p1, out Point p2)
        {
            ...
            {
                case JawPos.Left:
                    roi = new Rect((int)baseX + 2, basePt.Y - 175, 33, 50);
                    break;
                case JawPos.Right:
                    roi = new Rect((int)baseX - 35, basePt.Y - 175, 33, 50);
                    break;
                default:
                    roi = new Rect();
                    break;
        ...
        /// <param name="distance">0.013 溝深 量測值</param>
        /// <param name="correction">校正值</param>
        /// <param name="limitL">管制下限</param>
        /// <param name="limitU">管制上限</param>
        /// <returns>OK / NG</returns>
        public bool Cal013ThinknessValue(Mat src, Point basePoint, double compareX, out double toothX, out double distance, double correction = 0, double limitL = 0.006, double limitU = 0.010)
        {
            // 計算 roi
        ...
        /// <param name="X">ROI X (從前開取得)</param>
        /// <param name="cY">012 基準 1 (從輪廓度取得)</param>
        /// <param name="distance">(out) 012 量測距離</param>
        /// <param name="correction">校正值</param>
        /// <param name="limitL">管制下限</param>
        /// <param name="limitU">管制上限</param>
        /// <returns>
        ///     <strong>是否合格</strong>
        /// </returns>
        public bool Cal012DistanceValue(Mat src, Point basePoint, JawPos roiPos, double X, double cY, out double distance, double correction = 0, double limitL = 0.011, double limitU = 0.013)
        {
            // 計算 roi
            Rect roi = new Rect();
            ...
            // 銷毀 canny
            canny.Dispose();
            // Debug.WriteLine($"{roiPos} 012 botY: {botY}");
        ...
        [Obsolete("deprecated")]
        public bool Cal012DistanceValue_deprecated(Mat src, Point basePoint, JawPos roiPos, double X, double cY, out double distance, double correction = 0, double limitL = 0.011, double limitU = 0.015)
        {
            // 計算 roi
            Rect roi = new Rect();
            ...
            {
                case JawPos.Left:
                    roi = new Rect((int)X, basePoint.Y - 10, 33, 20);
                    break;
                case JawPos.Right:
                    roi = new Rect((int)X - 32, basePoint.Y - 10, 33, 20);
                    break;
                default:
                    break;
            }
        ...
        /// <param name="limitU">管制上限</param>
        /// <returns></returns>
        public bool Cal174DistanceValue(Mat src, double baseJigY, double compareX, JawPos leftRight, out double distance, double correction = 0, double limitL = 0.0855, double limitU = 0.0905)
        {
            // src Width
            int srcWidth = src.Width;
            // roi 距離影像邊緣 X
            ...
            // foreach (var line in lineV)
            //{
            //    Cv2.Line(src, line.P1, line.P2, Scalar.Gray, 2);
            //}
        ...
        /// <param name="baseJigY">治具邊緣</param>
        /// <param name="compareX">比較基準 (量測後開取得)</param>
        /// <param name="pos">左: Left, 右: Right</param>
        /// <param name="distance">(out) 0.174 距離</param>
        /// <param name="correction">校正值</param>
        /// <param name="limitL">管制下限</param>
        /// <param name="limitU">管制上限</param>
        /// <returns>是否合格</returns>
        public unsafe bool Cal174DistanceValue2(Mat src, double baseJigY, double compareX, JawPos pos, out double distance, double correction = 0, double limitL = 0.170, double limitU = 0.178)
        {
            // src Width
            int srcWidth = src.Width;
            // roi 距離影像邊緣 X
            ...
            {
                case JawPos.Left:
                    roi = new Rect(roiX, (int)(baseJigY - 150), roiWidth, 140);
                    iStart = 0;
                    iStep = 1;
                    break;
                case JawPos.Right:
                    roi = new Rect(srcWidth - roiX - roiWidth, (int)(baseJigY - 150), roiWidth, 140);
                    iStart = roi.Width - 1;
                    iStep = -1;
                    break;
            }
            ...
            for (int j = 0; j < roiMat.Height; j += 3)
            {
                grayArr = new double[roiMat.Width];
                grayAbs = 0;
                tempX = 0;

                for (int i = iStart; pos == JawPos.Left ? j < roiMat.Height : j >= 0; i += iStep)
                {
                    double avg = (b[srcWidth * j + i] + b[srcWidth * (j + 1) + i] + b[srcWidth * (j + 2) + i]) / 3;
                    grayArr[i] = avg;

                    int k = pos == JawPos.Left ? i - 1 : i + 1;
                    if (i == 0 || i == roiWidth - 1) { continue; }

                    if (grayArr[i] < grayArr[k] && Math.Abs(grayArr[i] - grayArr[k]) > grayAbs)
                    {
                        tempX = i;
                        grayAbs = Math.Abs(grayArr[i] - grayArr[k]);
                    }

                    if (avg < 20) { break; }
                }

                if (listX.Count == 0 || Math.Abs(tempX - listX[^1]) < 3)
                {
                    listX.Add(tempX);

#if DEBUG || debug
                    b[srcWidth * j + tempX] = 0;
                    b[srcWidth * j + tempX + 1] = 0;
                    b[srcWidth * j + tempX + 2] = 0;
                    b[srcWidth * j + tempX - 1] = 0;
                    b[srcWidth * j + tempX - 2] = 0;
#endif
                }
...
#if DEBUG || debug
                    b[srcWidth * j + tempX] = 150;
                    b[srcWidth * j + tempX + 1] = 150;
                    b[srcWidth * j + tempX + 2] = 150;
                    b[srcWidth * j + tempX - 1] = 150;
                    b[srcWidth * j + tempX - 2] = 150;
#endif
                }
        ...
        public void GetPomDatum(Mat src, out double datumY)
        {
            ...
            Methods.GetRoiCanny(src, roi, 30, 75, out Mat canny);
        ...
        public unsafe bool Cal007FlatnessValue4(Mat src, double baseDatumY, out double flatValue, double correction = 0, double limitU = 0.007)
        {
            ...
            {
                // 避開 pin
                if (i is < 2120 or >= 2330 && i < roiMat.Width)
                {
                    grayArr = new double[roiMat.Height];
                    tmpGrayAbs = 0;
                    tmpY = 0;
